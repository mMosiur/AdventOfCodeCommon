	...
	where T : unmanaged, INumber<T>
{
	...
	public static Point2D<T> Parse(ReadOnlySpan<char> s, IFormatProvider? provider = null)
	{
		...
		catch (Exception e)
		{
			...
			throw new FormatException($"Could not parse \"{s}\" as a point.", e);
		}
	}
	...
	/// the string representations of the <see cref="X"/> and <see cref="X"/> values.
	/// </remarks>
	public static bool TryParse([NotNullWhen(true)] string? s, [MaybeNullWhen(false)] out Point2D<T> result)
	...
	/// the string representations of the <see cref="X"/> and <see cref="X"/> values.
	/// </remarks>
	public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, [MaybeNullWhen(false)] out Point2D<T> result)
	{
		ArgumentNullException.ThrowIfNull(s);
	...
	/// the string representations of the <see cref="X"/> and <see cref="X"/> values.
	/// </remarks>
	public static bool TryParse([NotNullWhen(true)] ReadOnlySpan<char> s, [MaybeNullWhen(false)] out Point2D<T> result)
		=> TryParse(s, null, out result);
	...
	/// the string representations of the <see cref="X"/> and <see cref="X"/> values.
	/// </remarks>
	public static bool TryParse([NotNullWhen(true)] ReadOnlySpan<char> s, IFormatProvider? provider, [MaybeNullWhen(false)] out Point2D<T> result)
	{
		const int CoordCount = 2;
		const char Separator = ',';
		const char OpenChar = '(';
		...
		result = new Point2D<T>(values[0], values[1]);
		return true;
	}
